...
public class ExtResponseTemplateTransformer extends ResponseDefinitionTransformer implements StubLifecycleListener {

    public static final String NAME = "ext-response-template";
    ...
    public ExtResponseTemplateTransformer(boolean global) {
        this(global, Collections.<String, Helper>emptyMap());
    }

    public ExtResponseTemplateTransformer(boolean global, String helperName, Helper helper) {
        this(global, ImmutableMap.of(helperName, helper));
    }

    public ExtResponseTemplateTransformer(boolean global, Map<String, Helper> helpers) {
        this(global, new Handlebars(), helpers, null, null);
    }

    public ExtResponseTemplateTransformer(boolean global, Handlebars handlebars, Map<String, Helper> helpers, Long maxCacheEntries, Set<String> permittedSystemKeys) {
        this.global = global;
    ...
    @Override
    public ResponseDefinition transform(Request request, final ResponseDefinition responseDefinition, FileSource files, Parameters origParameters) {
        ResponseDefinitionBuilder newResponseDefBuilder = ResponseDefinitionBuilder.like(responseDefinition);

        final Parameters newParameters = uncheckedApplyTemplate(request, responseDefinition, files, origParameters);

        final ImmutableMap<String, Object> model = ImmutableMap.<String, Object>builder()
                .put("parameters", firstNonNull(newParameters, Collections.<String, Object>emptyMap()))
                .put("request", RequestTemplateModel.from(request))
                .putAll(addExtraModelElements(request, responseDefinition, files, newParameters))
                .build();
        ...
        if (responseDefinition.specifiesTextBodyContent()) {
            ...
            boolean disableBodyFileTemplating = newParameters.getBoolean("disableBodyFileTemplating", false);
    ...
    private HandlebarsOptimizedTemplate getTemplate(final TemplateCacheKey key, final String content) {
        return getTemplateObj(key, content);
    }
    ...
    public static class Builder {
        ...
        public ExtResponseTemplateTransformer build() {
            return new ExtResponseTemplateTransformer(global, handlebars, helpers, maxCacheEntries, permittedSystemKeys);
        }
    }
